    ...
    $(document).ready(function () {
        ...
        function setYoutubeLinks(youtubeLinks){
            youtubeLinks.each(function (index, link) {
                if($(link).find('.video-submit input:nth-child(1)').val() && $(link).find('.video-submit input:nth-child(2)').val()){
                    let duration = $(link).find('.video-submit input:nth-child(2)').val();
                    if(duration > 0){
                        var youTubeText ;
                        if($(link).find('.video-submit').parent().find('.text-youtube').length>0){
                            youTubeText = $(link).find('.video-submit').parent().find('.text-youtube');
                        }
                        else {
                            var divText = '<div class="text-youtube"></div>';
                            $(divText).appendTo( $(link).find('.video-submit').parent());
                            youTubeText = $(link).find('.video-submit').parent().find('.text-youtube');
                        }
        ...
        $('.vacancy-settings').on('click', '.add-new-youtube-link', function () {
            ...
            input.name = 'youtube-link_' + id_block + '_'+ id_lesson +'_'+ id_link;
        ...
        $('.remove-popup').on('click', '.submit-remove-course', function () {
            ...
            if ($(this).data('type') == 'inputfile') {
                if($('#' + id).parents('.course-materials-wrap').length>0){
                    ...
                    let materialsCont = $('#' + id).parents('.vac-settings__materials');
                    if(courseId && blockId && lessonId && materialId){
                        $.ajax ({
                            ...
                            checkMaterialsLength(materialsCont);
        ...
        //youtube duration
        $('.vacancy-settings').on('input keydown keyup mousedown mouseup select contextmenu drop change', '.video-submit input[type=text]', function(e){
            var url = $(this).val();
            var vidid = youtube_parser(url);
            var YOUR_API_KEY = "AIzaSyBNhDxp4JZ1B6RHz3YT3yZo_R-yfINbw0o";
            var inDuration = $(this).parents('.video-submit').find('.youtube-duration');
            var youTubeText ;
            if($(this).parents('.video-submit').parent().find('.text-youtube').length>0){
                youTubeText = $(this).parents('.video-submit').parent().find('.text-youtube');
            }
            else {
                var divText = '<div class="text-youtube"></div>';
                $(divText).appendTo( $(this).parents('.video-submit').parent());
                youTubeText = $(this).parents('.video-submit').parent().find('.text-youtube');
            }
            if(vidid){
                $.ajax({
